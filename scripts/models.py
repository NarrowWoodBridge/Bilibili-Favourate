...
class Config:
    ...
    syncFolderList: list = field(default_factory=list)  # 要抓取的收藏夹的名称列表
    ...
    def __post_init__(self):
        ...
        self.syncFolderList = [i for i in names if i != '']  #去除空项目
...
class User:
    ...
    def __post_init__(self):
        #获取要同步的收藏夹内视频信息
        favFolders = getFavFolders(self)  #存储账号上所有收藏夹的信息
        for tSyncFolder in self.config.syncFolderList:
            print("查询收藏夹："+tSyncFolder)
            if not tSyncFolder in favFolders:
                print("不存在此收藏夹："+tSyncFolder)
                continue
